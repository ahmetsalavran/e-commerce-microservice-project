...
ALTER TABLE products
  ADD COLUMN IF NOT EXISTS available INTEGER NOT NULL DEFAULT 1;

TRUNCATE TABLE products RESTART IDENTITY;

INSERT INTO products (product_id, sku, name, base_price, available, updated_at)
SELECT
  gs::bigint AS product_id,
  'PRD-' || lpad(gs::text, 3, '0') AS sku,
  'Product ' || gs AS name,
  (50 + random() * 4950)::numeric(18,2) AS base_price,
  (100 + random() * 99900)::int AS available,
  now() AS updated_at
