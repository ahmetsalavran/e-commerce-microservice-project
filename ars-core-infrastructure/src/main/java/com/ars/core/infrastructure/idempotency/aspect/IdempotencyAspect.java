...
public class IdempotencyAspect {
    ...
    public Object around(ProceedingJoinPoint pjp, Idempotent idempotent) throws Throwable {
        ...
        if (idempotent.mandatoryTx() && !TransactionSynchronizationManager.isActualTransactionActive()) {
            throw new IllegalStateException("@Idempotent requires an active transaction (mandatoryTx=true).");
        }
        ...
        if (eventId == null || eventId.isBlank()) {
            throw new IllegalStateException("Idempotency key resolved to blank. Check @Idempotent(key=...)");
        }
